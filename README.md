...
## ⚠️ Многопоточная загрузка в форках Telegram

Форки клиента (ExteraGram, Nagram и другие) умеют «ускорять загрузку»: файл
дробится на куски по мегабайту и тянется дюжиной потоков сразу. На прямом
соединении это выигрыш, а через прокси каждый поток — отдельная TLS-сессия,
которую надо поднять одновременно с остальными. Клиент столько не ждёт: он
отменяет запросы, ответы приезжают уже ненужными (`received chunk but definitely
cancelled` в его логе), видео не проигрывается, а сам прокси при этом получает
репутацию сломанного — вплоть до сообщения «прокси настроен неверно и будет
отключён».

Если тяжёлые файлы не грузятся, а переписка и картинки работают, — выключите
ускоритель: **ExteraGram → Настройки → ExteraGram → Скорость загрузки → Выкл.**
В официальном клиенте такого режима нет, и через тот же прокси он качает
нормально; это быстрый способ отличить проблему клиента от проблемы прокси.

## ⚠️ Старый секрет в другом клиенте

Если секрет прокси сменился (переустановка, сброс настроек), клиент, где остался
старый, продолжает долбиться в него без остановки: Telegram не отличает неверный
секрет от недоступного сервера и переподключается сразу после каждого отказа —
на практике это сотни попыток в секунду. Рабочий клиент при этом начинает
отваливаться.

Приложение показывает про это баннер и даёт скопировать актуальную ссылку.
Достаточно заново добавить прокси в том клиенте (или выключить его там).
